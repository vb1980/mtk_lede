...
static struct wifi_dev *get_wdev_by_devinfo(struct netdev_info *devinfo)
{
	...
	netdev = dev_get_by_index(&init_net, devinfo->ifindex);
	MTWF_DBG(NULL, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_INFO,
		":%s: netdev:%p\n", __func__, netdev);
	if (netdev == NULL)
		return NULL;
...
int bssmnger_is_entry_exist_2g_5g(void)
{
	...
	DlListForEach(bmgentry, entry_list, struct bmg_entry, list) {
		...
		if (WMODE_CAP_2G(radioinfo->phymode) || WMODE_CAP_5G(radioinfo->phymode))
			ap_non_6g_entry_cnt++;
		else if (WMODE_CAP_6G(radioinfo->phymode))
			ap_6g_entry_cnt++;
		else {
			MTWF_DBG(NULL, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
...
bool bssmnger_update_6g_iob_rule(struct bmg_entry *target_bmgentry)
{
	...
	DlListForEach(curr_bmgentry, entry_list, struct bmg_entry, list) {
		...
		if (!bssmnger_is_entry_exist_2g_5g() &&
			...
			MTWF_DBG(NULL, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				"<- iob dsc enable: curr iob_type (%d) change (%d)\n",
...
NDIS_STATUS bssmnger_dereg_bmg_entry(struct wifi_dev *wdev)
{
	...
	DlListForEach(bmgentry, entry_list, struct bmg_entry, list) {
		...
		entry_devinfo = &bmgentry->devinfo;
		if (entry_devinfo == NULL) {
			ret = NDIS_STATUS_INVALID_DATA;
			MTWF_DBG(NULL, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				":%s: devinfo is NULL!! (ret:%d)\n", __func__, ret);
			continue;
		}
		...
		if (is_same_netdevinfo(entry_devinfo, &devinfo)) {
...
err:
	...
	if (bmgentry) {
		if (bssmnger_update_6g_iob_rule(bmgentry))
			bssmnger_update_reported_bss_list(bmgentry, TRUE);
	}
	MTWF_DBG(NULL, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_INFO,
...
err:
	MTWF_DBG(NULL, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_INFO,
		":%s: init %s, kernel_band_bmap:0x%x, entry_len:%d, inited:%d, devCnt:%d\n",
		__func__,  ret == NDIS_STATUS_SUCCESS ? "done":"fail",
		DlListLen(entry_list), bssmnger.inited, bssmnger.kernel_band_bitmap,
		bssmnger.dev_cnt);
	return ret;
}
...
NDIS_STATUS bssmnger_deinit(void)
{
	...
	bssmnger.inited = FALSE;
	bssmnger.kernel_band_bitmap = 0x00;
...
err:
	MTWF_DBG(NULL, DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_INFO,
		":%s: deinit %s, entry_len:%d, inited:%d, devCnt:%d\n",
		__func__,  ret == NDIS_STATUS_SUCCESS ? "done":"fail",
		DlListLen(entry_list), bssmnger.inited, bssmnger.dev_cnt);
	return ret;
}
