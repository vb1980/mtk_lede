...
static UCHAR get_wifi_pcie_slot(struct _RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_HIF, CATHIF_PCI, DBG_LVL_OFF, ("===> %s(): name: %s\n", __func__, pdev->bus->name));
...
VOID mtf_set_conn_infra_sysram(struct _RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: infra_addr=0x%x, infra_val=0x%x\n",
			__func__, ADDR_CONN_INFRA_SYSRAM, VAL_CONN_INFRA_SYSRAM));
}
...
VOID mtf_get_wm_pc_status(struct _RTMP_ADAPTER *pAd)
{
	...
	if (pAd->coex_pre_wm_pc == wm_pc) {
		...
		__func__, 0x7c060204, wm_pc, wm_pc_same_count));
		if (wm_pc_same_count % 10 == 0) {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("%s:WM_PC[0x%x] = 0x%x has not changed within 3s!!\n",
			__func__, 0x7c060204, wm_pc));
			wm_pc_same_count = 0;
...
int send_wifi_info_to_wifi_md_coex(struct _RTMP_ADAPTER *pAd, BOOL status)
{
	...
	MTWF_DBG(pAd, DBG_CAT_INIT, DBG_SUBCAT_ALL, DBG_LVL_NOTICE, "pAd=%p, status=%d\n", pAd, status);
	...
	if (card_type == 0) {
		MTWF_LOG(DBG_CAT_INIT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: card_type == 0.\n", __func__));
		return FALSE;
	}

	pci_num = mt_get_active_pci_num();

	MTWF_LOG(DBG_CAT_INIT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: pci_slot_id=%d, card_type=%d, pci_num=%d\n", __func__, pci_slot_id, card_type, pci_num));
