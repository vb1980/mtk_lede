...
static BOOLEAN get_chn_bitmask_pos(UCHAR channel, UCHAR *ch_type_idx, UCHAR *ch_bit_pos)
{
	...
	if (channel <= SAFE_CHN_END_IDX_2G4) {
		ch_step = 1;
...
static UINT32 get_first_safe_channel(PRTMP_ADAPTER pAd, UCHAR band_idx)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_UNSAFE, DBG_LVL_TRACE, "band_idx=%d\n", band_idx);
...
static UCHAR select_safe_channel(PRTMP_ADAPTER pAd, UCHAR band_idx)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_UNSAFE, DBG_LVL_TRACE, "band_idx=%d, new_chn=%d\n", band_idx, new_chn);
	return new_chn;
}
	...
	UINT32 *channel_bit_mask, UCHAR *change_type)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_UNSAFE, DBG_LVL_TRACE, "channel_bit_mask=%x-%x-%x\n",
	...
	*change_type = SAFE_CHN_TYPE_NONE;
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_UNSAFE, DBG_LVL_TRACE, "SafeChnBitmask=%x-%x-%x, bChanged=%d(%d)\n",
		pAd->LteSafeChCtrl.SafeChnBitmask[0],
...
static VOID band_turn_into_all_unsafe(PRTMP_ADAPTER pAd, struct wifi_dev *wdev, UCHAR band_idx)
{
	...
	NdisReleaseSpinLock(&pAd->LteSafeChCtrl.SafeChDbLock);
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_UNSAFE, DBG_LVL_OFF, "All channels are unsafe now! Stay in CurrentChannel %d.\n", wdev->channel);
...
static VOID switch_unsafe_channel(PRTMP_ADAPTER pAd, struct wifi_dev *wdev, UCHAR band_idx)
{
	...
	if (!rtmp_set_channel(pAd, wdev, new_channel))
		...
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_UNSAFE, DBG_LVL_TRACE, "rtmp_set_channel succeed: band_idx(%d), new_channel(%d).\n",
		band_idx, new_channel);
...
static BOOLEAN is_all_available_chn_unsafe(PRTMP_ADAPTER pAd, UCHAR band_idx)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_UNSAFE, DBG_LVL_TRACE, "band_idx=%d, b_all_unsafe=%d\n", band_idx, b_all_unsafe);
	return b_all_unsafe;
}
...
static BOOLEAN is_chn_op_state_idle(PRTMP_ADAPTER pAd, struct wifi_dev *wdev)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_UNSAFE, DBG_LVL_TRACE, "\n");
	if (scan_in_run_state(pAd, wdev)) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_UNSAFE, DBG_LVL_TRACE, "Scan is running now!\n");
		return FALSE;
	}
	...
	if (pDot11h) {
		if (pDot11h->RDMode != RD_NORMAL_MODE) {
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_UNSAFE, DBG_LVL_TRACE, "RDMode=%d!\n", pDot11h->RDMode);
			return FALSE;
		}
	}
...
VOID band_safe_chn_event_process(RTMP_ADAPTER *pAd, UCHAR band_idx)
{
	...
	if (need_enqueue) {
		...
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_UNSAFE, DBG_LVL_TRACE, "Enqueue cmd for band(%d). \n", band_idx);
	}
}
...
VOID LteSafeBuildChnBitmask(PRTMP_ADAPTER pAd, UINT8 band_idx)
{
	...
	/* DBDC mode cannot clear AvaChnBitmask to avoid affecting other bands */
...
BOOLEAN IsChannelSafe(PRTMP_ADAPTER pAd, UCHAR channel)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_UNSAFE, DBG_LVL_TRACE, "channel=%d\n", channel);
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_UNSAFE, DBG_LVL_TRACE, "Channel(%d) ch_type_idx(%d) ch_bit_pos(%d) is %s.\n",
...
VOID LteSafeChnEventHandle(RTMP_ADAPTER *pAd, UINT32 *channel_bit_mask)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_UNSAFE, DBG_LVL_TRACE, "FW_event_into_channel_bit_mask=%x-%x-%x\n",
	...
	for (band_idx = DBDC_BAND0; band_idx < DBDC_BAND_NUM; band_idx++) {
		...
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_UNSAFE, DBG_LVL_TRACE, "band_idx=%d, band_chn_type=%d, chn_chg_type=%d\n",
...
NTSTATUS LteSafeChannelChangeProcess(PRTMP_ADAPTER pAd, PCmdQElmt CMDQelmt)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_UNSAFE, DBG_LVL_TRACE, "band_idx=%d, dbdc_mode=%d\n", band_idx,
	...
	for (i = 0; i < WDEV_NUM_MAX; i++) {
		...
		if (wdev != NULL) {
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_UNSAFE, DBG_LVL_TRACE, "type=%d, state=%d, band_idx=%d\n",
	...
	if (WDEV_BSS_STATE(wdev) < BSS_READY) {
		if (!pAd->LteSafeChCtrl.bAllUnsafe[band_idx]) {
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_UNSAFE, DBG_LVL_TRACE, "wdev(%d) bss not ready (state:%d)!!\n",
					 wdev->wdev_idx, WDEV_BSS_STATE(wdev));
			goto end;
		} else
			MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_UNSAFE, DBG_LVL_TRACE, "In all unsafe case, band_idx=%d\n",
			band_idx);
	}
	...
	if (!is_chn_op_state_idle(pAd, wdev) || pAd->IoctlHandleFlag || pAd->ApCfg.iwpriv_event_flag) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_UNSAFE, DBG_LVL_TRACE, "Other channel op is running on wdev(%d), IoctlHandleFlag=%d, iwpriv_event_flag=%d!!\n",
	...
	if (!TakeChannelOpCharge(pAd, wdev, CH_OP_OWNER_LTE_SAFE_CHN, FALSE)) {
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_UNSAFE, DBG_LVL_TRACE, "TakeChannelOpCharge fail for safe channel!!\n");
		goto end;
	}
	...
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_UNSAFE, DBG_LVL_TRACE, "Channel operation type on wdev(%d) is %d.\n",
	...
	if (!bOpDone) {
		...
		MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_UNSAFE, DBG_LVL_TRACE, "(band_idx=%d), Not processed, line up again. \n", band_idx);
	} else
		ReleaseChannelOpCharge(pAd, wdev, CH_OP_OWNER_LTE_SAFE_CHN);

	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_UNSAFE, DBG_LVL_TRACE, "end. \n");
